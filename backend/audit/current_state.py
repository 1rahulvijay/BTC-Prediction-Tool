"""Generate docs/active/SOURCE_STATE.json and .md FROM THE SOURCE, and fail CI when stale.
...
#: SOURCE_STATE, not CURRENT_STATE. The distinction is not pedantry: this document is derived
#: from source and is therefore self-consistent by construction, which is exactly why it
#: published `TIE_RESOLVES_TO = DOWN` with complete confidence while the venue resolves ties
#: UP. A source-consistent document can still be semantically wrong, and calling it "current
#: state" invited it to be read as ground truth about a running system.
#:
#: RUNTIME_STATE (current release, artifact hashes, serviceability, recorder health) and
#: RESEARCH_STATE (valid/retracted lanes, forward counts, promotion candidates) are separate
#: documents with separate owners, because they go stale on their own schedules and a
#: staleness check that fires for the wrong reason is one people learn to skip.
OUT_JSON = REPO / "docs" / "active" / "SOURCE_STATE.json"
OUT_MD = REPO / "docs" / "active" / "SOURCE_STATE.md"
...
def collect() -> dict:
    ...
    return {
        ...
        "target_contracts": {
            ...
            "proxy": sorted(tc.PROXY_SETTLEMENT_CONTRACTS),
            ...
            "settlement_rule": tc.DEFAULT_SETTLEMENT_RULE.identity(),
        },
...
def render_markdown(state: dict) -> str:
    lines = [
        ...
        f"- settlement rule: `{state['target_contracts']['settlement_rule']['rule_version']}` "
        f"({state['target_contracts']['settlement_rule']['comparator']}, tie -> "
        f"**{state['target_contracts']['settlement_rule']['tie_outcome']}**)",
        f"- market settles on `{state['target_contracts']['settlement_rule']['source']}`; "
        f"our labels observe `{state['target_contracts']['settlement_rule']['observed_source']}` "
        f"-> **proxy: {state['target_contracts']['settlement_rule']['is_proxy']}**",
        "",
...
def check() -> int:
    ...
    if stale:
        ...
        print("  code moves.")
        return 1
    print(f"SOURCE STATE: fresh ({len(state['source_digests'])} sources hashed, "
