    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            if isinstance(v, (int, float)) and 1000 < float(v) < 10_000_000:
                ...
                match = re.search(r"\$?\s*([0-9]{2,3}(?:,[0-9]{3})+(?:\.[0-9]+)?)", v)
                if match and any(
                    x in key_lower
                    for x in (
                        "strike",
...
def try_chainlink_polygon() -> dict:
    ...
    for rpc in POLYGON_RPCS:
        try:
            ...
            response = requests.post(rpc, json=payload, timeout=15)
            result["ms"] = int((time.time() - start) * 1000)
...
def try_scrape(window_start: int, retries: int = 3) -> dict:
    ...
    for attempt in range(1, retries + 1):
        ...
        try:
            ...
            last_error = e
            result["status"] = f"ERR attempt {attempt}: {str(e)[:80]}"
            time.sleep(5)
    ...
    result["status"] = f"FAILED: {str(last_error)[:80]}"
    return result
...
def reachability_check():
    ...
    for name, url in targets:
        try:
            ...
            print(f"{name:<18} reachable HTTP {response.status_code} {ms}ms")
        except Exception as e:
            print(f"{name:<18} BLOCKED/ERR {str(e)[:80]}")
...
def run_window(window_start: int, do_scrape: bool):
    ...
    # Source priority:
    # 1. Scrape = exact Polymarket UI value
    # 2. Gamma = only if future field appears
    # 3. Chainlink on-chain = best free fallback
    # 4. Pyth = fallback
    # 5. Binance boundary open = last fallback only
    ...
    print(
        f"GAMMA       : {gamma['status']:<28} price={gamma.get('price')} candidates={gamma.get('candidates')[:3]}"
    )
    print(f"SCRAPE      : {scrape['status']:<28} price={scrape.get('price')}")
    print(
        f"CHAINLINK   : {chainlink['status']:<28} price={chainlink.get('price')} rpc={chainlink.get('rpc')}"
    )
    print(f"PYTH        : {pyth['status']:<28} price={pyth.get('price')}")
    print(
        f"BINANCE     : {binance['status']:<28} live={binance.get('live')} boundary_open={binance.get('boundary_open')}"
    )
    print("-" * 80)
...
def main():
    ...
    parser.add_argument(
        "--scrape",
        action="store_true",
        help="Enable Playwright scrape of Polymarket page",
    )
    parser.add_argument("--once", action="store_true", help="Run once and exit")
    parser.add_argument(
        "--no-reachability", action="store_true", help="Skip reachability check"
    )
