...
start "BTC 180d Quantile Only" powershell -NoExit -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Continue'; Set-Location -LiteralPath '%PROJECT_ROOT%'; & '%PYTHON_EXE%' 'backend\research\train_360d_multitarget_forecaster.py' --days 180 --horizons 5 15 --models 'lightgbm' --device gpu --skip-regression --skip-classification --skip-sequence --quantile-backends lightgbm --output-prefix forecast_180d_quantile_only --max-features 160 --n-jobs 2 2>&1 | Tee-Object -FilePath '%LOG_FILE%'"
