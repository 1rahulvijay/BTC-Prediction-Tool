...
| `MODEL_CROSSFLIP_L1_V1` + `_L2_V1` | **Sequential two-leg (operator idea, 2026-07-04).** Leg 1: path head says two_sided + round-trip ≥30% → buy the leader mid-window (25–85% of the round) at ask 0.50–0.85, spread ≤3c, depth ≥1, **and only when the market leader (higher bid) agrees with the BTC leader (price vs anchor)** — the two definitions differ, and without that check leg 2 could fire on a disagreement instead of a real cross. Leg 2: **only if the anchor is then actually crossed** (cur_pos flips) with >20s left and ask ≤0.85 → buy the NEW leader too. Each leg is an independent ledger row settled on its own; **the strategy is the SUM of L1+L2**. Versus the straddle: legs are staggered at different prices and quiet/trend rounds cost ONE leg instead of two. Guard logic smoke-tested (11/11 cases). |
