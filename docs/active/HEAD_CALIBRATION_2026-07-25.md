...
## VERDICT - and why it matters more than any strategy result

| head | skill (BSS) | calibration | usable? |
|---|---|---|---|
| **P(Hold)** | +0.044 | **over-confident by 6.7pp** | ranks weakly; **scale is wrong** |
| Flip risk | +0.002 | under-predicts low bands badly | ~no skill |
| Late shock >=$20 | **-0.013** | over-confident above 40% | **no skill** |
| Late shock >=$50 | +0.032 | fair below 30% | marginal |
| Late shock >=$100 | +0.068 | fair (base rate 0.2%) | marginal, almost never fires |
| Champion tiers | - | **NOT monotone** | **must not be shown as a ranking** |

### 1. P(Hold) is over-confident by more than the entire edge

Predicted **96.1%** vs realized **89.3%** - a **6.7pp** gap, worst exactly where the app lives: the
95-100% band holds **5,512 of 6,727** rounds and realizes **93.4%** against a predicted 99.6%.
Mid-bands are worse (85-90% predicts 88.0%, realizes 72.3%).

This is not cosmetic. The champion prices fair value as `min(P(Hold), 0.91)` and the frozen rule
gates on `P(Hold) >= 0.93`. **A 6.7pp bias in P(Hold) is a ~6.7c bias in fair value - about seven
times the rule's entire measured edge (+0.90c).** Every edge the app has computed inherited this
bias in the optimistic direction.

Note the 0.91 fair-value cap accidentally truncated exactly the over-confident region. That was
defensive luck, not calibration.

### 2. The champion's action tiers are INVERTED

`PAPER` (the act bucket) held **69.4%**; `WAIT` held **89.6%**. The tier presented as strongest is
the weakest by outcome. There is a benign mechanical explanation - PAPER fires when an executable
edge exists, i.e. when the ask is cheap, i.e. when the leader is least certain - but that is the
point: **the tier ranks cheapness, not confidence.** Per the stratifier rule (monotone, n>=100 per
level) it does not qualify to be displayed as a confidence ranking.

### 3. Flip risk and the $20 shock head carry no usable information

BSS +0.002 and **-0.013**. At or below zero means the head ranks no better than always predicting
the base rate, whatever its training AUC was. The $20 head is also over-confident where it matters:
its 90-100% band realizes **60.6%**.

### What follows

- **No new strategy should be built on P(Hold), flip risk or the $20 shock head until they are
  recalibrated** on live outcomes (isotonic on the deployment set, refit per bundle).
  Recalibration, not replacement - P(Hold) does carry weak positive skill; it is mis-scaled.
- **Remove or relabel the champion tier display** until it stratifies monotonically.
- This reframes the late-leader result: the rule was never trading a 95% edge. It traded a ~93%
  reality priced as if it were 99.6%, and the measured +0.90c already contains that error - one
  more reason the honest reading is "approximately zero".

### A measurement bug found and fixed inside this test

The first run reported the late-shock heads ~20x under-confident (predicted 1.4%, realized 31.5%).
That was **this test's error, not the model's**: the heads predict movement *remaining from the
checkpoint* while the label used the *total round move from the anchor* - a larger quantity over a
longer interval. Corrected to `|move - current_move|` (both anchor-relative, so the difference is
the post-checkpoint move), the same heads read marginal-but-sane. An implausible result is a bug
