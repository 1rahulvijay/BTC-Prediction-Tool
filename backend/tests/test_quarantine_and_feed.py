...
diagnostics must never be able to stall the feed that produces them.
    ...
    python backend/tests/test_quarantine_and_feed.py
"""
...
def test_legacy_algodesk_quarantined() -> None:
    """backend/research/standalone/algodesk_ml_rl_dl.py fabricates funding and open interest.
...
def test_canonical_algodesk_does_not_fabricate() -> None:
    ...
    agents = Path(__file__).resolve().parents[2] / "research" / "algodesk" / "agents.py"
