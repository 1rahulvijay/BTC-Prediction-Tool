"""Mandatory CLI and report lifecycle used by every standalone experiment."""
...
from . import (block_bootstrap, causal_loader, cost_model, matched_controls, metrics,
               modeling, protocol, temporal_split)
from .engine_types import EngineContext
from .engines import ENGINES, execute
from .report_writer import (git_commit, git_worktree_dirty, source_tree_hash,
                            write_report)
...
def parser_for(experiment_id: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=f"Phase 5 standalone: {experiment_id}")
    parser.add_argument("--data-dir", default=str(REPO / "data"))
    parser.add_argument("--output", default=str(REPO / "data" / "research" /
                                                 "phase5_standalone" / experiment_id / "latest"))
    ...
    parser.add_argument("--maximum-rows", type=int, default=100_000,
                        help="Newest rows to load; 0 means no cap.")
    return parser
...
def _selftest(protocol_path: Path) -> int:
    ...
    metrics.selftest()
    modeling.selftest()
    with tempfile.TemporaryDirectory(prefix="phase5-selftest-") as temp:
        causal_loader.selftest(Path(temp))
...
def run_cli(protocol_path: str | Path, argv: list[str] | None = None) -> int:
    ...
    payload = {
        ...
        "suite_code_sha256": source_tree_hash(REPO / "research" / "phase5_standalone"),
