...
start "BTC 360d Advanced Sequence" powershell -NoExit -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Continue'; Set-Location -LiteralPath '%~dp0'; & '%PYTHON_EXE%' 'backend\research\train_180d_advanced_sequence_models.py' --days 360 --horizons 5 15 --models 'vlstm,lpatchtst,patchtst,itransformer' --device gpu --max-features 96 --seq-len 60 --seq-max-rows 80000 --batch-size 256 --epochs 5 --output-prefix forecast_360d_advanced_sequence 2>&1 | Tee-Object -FilePath '%LOG_FILE%'"
